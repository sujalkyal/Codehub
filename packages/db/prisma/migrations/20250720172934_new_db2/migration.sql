/*
  ...
  - You are about to drop the column `testCaseId` on the `SubmissionTestCaseResult` table. All the data in the column will be lost.
  ...
  - A unique constraint covering the columns `[submissionId]` on the table `SubmissionTestCaseResult` will be added. If there are existing duplicate values, this will fail.
...
-- DropIndex
DROP INDEX "SubmissionTestCaseResult_submissionId_testCaseId_key";

-- AlterTable
ALTER TABLE "SubmissionTestCaseResult" DROP COLUMN "testCaseId";
...
-- CreateIndex
CREATE UNIQUE INDEX "SubmissionTestCaseResult_submissionId_key" ON "SubmissionTestCaseResult"("submissionId");
